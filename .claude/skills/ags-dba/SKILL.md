...
- Cada serviço (`src/Services/<NomeDoServico>Service/`) tem seu próprio **banco lógico** na instância Postgres, nomeado `ouroboros_<nomedoservico>` (ex.: serviço `Auth` → banco `ouroboros_auth`), com uma *role* própria que é dona desse banco. É isso — banco físico, não só schema — que garante o isolamento entre serviços: nenhum serviço tem credencial que alcance o banco de outro.
