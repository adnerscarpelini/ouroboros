...
7. Configure Serilog + Seq e o `GlobalExceptionHandler`, seguindo exatamente o padrao do `auth-service` (ver secao 4 acima e `docs/0003 - Logging e Erros.md`).
8. Crie `{Servico}.Api/Dockerfile` e adicione o servico como um novo `service` no `docker-compose.yml` da raiz, incluindo a variavel `Seq__ServerUrl: http://seq:5341` no `environment` (ver `docs/0002 - Docker.md`).
