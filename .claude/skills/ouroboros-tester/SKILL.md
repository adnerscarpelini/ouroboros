...
3. **Entidade de dominio com validacao propria** → se essa logica ja e inteiramente exercida pelos testes do caso de uso que a usa, nao duplique o teste. Se a entidade tiver uma regra que nenhum caso de uso testado hoje exercita, escreva um teste direto da entidade, no projeto `{Servico}.Domain.Tests` (mesma pasta paralela `test/{servico}-service/`, referenciando so `{Servico}.Domain`), classe `{Entidade}Tests`, espelhando a pasta `Entities/` do dominio.
...
- Fica no projeto `{Servico}.Application.Tests`, dentro de `test/{servico}-service/` — uma pasta paralela na raiz do monorepo, **fora** de `{servico}-service/` (que so tem o que roda em producao/imagem Docker). O projeto de teste referencia `{Servico}.Application` do servico correspondente via `ProjectReference` relativo (`../../../{servico}-service/{Servico}.Application/...`). Mesma estrutura de pasta do caso de uso dentro do projeto de teste (`UseCases/{CasoDeUso}/`), classe `{CasoDeUso}InteractorTests`.
