...
- Implementação mora em `Infrastructure`, pasta `Queries/`, usando Dapper com SQL escrito à mão, contra o schema do próprio módulo (schema explícito na query, já que Dapper não conhece o `HasDefaultSchema` configurado no `DbContext`).
- Registro no mesmo `Add<NomeDoModulo>Module` onde hoje entram os serviços, como `services.AddScoped<I<Nome>Query, <Nome>Query>();`.
...
Dapper precisa de uma `IDbConnection` própria, fora do `DbContext`. Um `IDbConnectionFactory` (contrato em `Ouroboros.Common.Application`, implementação Npgsql em `Ouroboros.Common.Infrastructure`) fica disponível pra qualquer módulo pedir uma conexão pra Dapper, reaproveitando a mesma connection string já usada pelo EF Core — sem duplicar configuração ou segredo.
...
- O isolamento entre módulos continua valendo: um Query Object só lê do schema do próprio módulo.

## Consequências

- Duas formas de acessar o banco convivem por módulo quando necessário (EF Core pra escrita, Dapper pra leitura pesada) — mais uma peça pra manter, só onde compensar.
...
- `Common.Infrastructure` ganha uma peça de infraestrutura genérica (`IDbConnectionFactory`) só quando o primeiro módulo realmente precisar dela — não é criada agora, por antecipação.
