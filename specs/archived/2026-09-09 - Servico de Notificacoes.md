...
- Outbox transacional em `BuildingBlocks`: `OutboxMessage`, `IOutboxMessageQueue`, `IMessagePublisher`, `OutboxPublisher<TDbContext>`, `OutboxPublisherProcessor<TDbContext>` e `AddTransactionalOutbox`.
...
- Não foi criado `Ouroboros.Services.Notifications.Api.Tests`: o host não tem nada além dos dois health checks.
...
| Persistência comum | [AppDbContext](../src/BuildingBlocks/Ouroboros.BuildingBlocks.Infrastructure/Persistence/AppDbContext.cs), [CommonEntityConfiguration](../src/BuildingBlocks/Ouroboros.BuildingBlocks.Infrastructure/Persistence/CommonEntityConfiguration.cs) | Manter colunas base e carimbos; tornar outbox optativa e retirar mapeamento universal de e-mail ao final. |
...
| Casos de uso produtores | [UserRegistrationService](../src/Services/Auth/Ouroboros.Services.Auth.Application/Services/UserRegistrationService.cs), [PasswordResetService](../src/Services/Auth/Ouroboros.Services.Auth.Application/Services/PasswordResetService.cs) | Preservar atomicidade e regras de token, substituir dependências de fila/template. |
| Identidade | [Token](../src/Services/Auth/Ouroboros.Services.Auth.Domain/Token.cs), [TokenRepository](../src/Services/Auth/Ouroboros.Services.Auth.Infrastructure/Persistence/Repositories/TokenRepository.cs), [AuthenticationService](../src/Services/Auth/Ouroboros.Services.Auth.Application/Services/AuthenticationService.cs) | Trocar correlação do token. Login, refresh, logout, hash de senha e JWT permanecem em Auth. |
| Banco de Auth | [AuthDbContext](../src/Services/Auth/Ouroboros.Services.Auth.Infrastructure/Persistence/AuthDbContext.cs), [UnitOfWork](../src/Services/Auth/Ouroboros.Services.Auth.Infrastructure/Persistence/UnitOfWork.cs), [migrations](../src/Services/Auth/Ouroboros.Services.Auth.Infrastructure/Migrations) | Adicionar outbox e correlação UUID antes de retirar legado; migrations independentes por banco. |
| Templates | [renderer](../src/Services/Auth/Ouroboros.Services.Auth.Infrastructure/Services/EmailTemplateRenderer.cs), [templates de e-mail](../src/Services/Auth/Ouroboros.Services.Auth.Infrastructure/Templates), [templates HTTP](../src/Services/Auth/Ouroboros.Services.Auth.Api/Templates) | Migrar apenas templates de e-mail. Páginas de sucesso/falha da confirmação ficam no Auth. |
...
`Token.EmailMessageId` é obrigatório no construtor, mas não tem navegação nem FK. Isso está explícito no domínio e na [InitialCreate](../src/Services/Auth/Ouroboros.Services.Auth.Infrastructure/Migrations/20260903001615_InitialCreate.cs). Não se deve substituir esse número por um ID interno do banco de Notificações.
...
`AuthDbContext` mapeia negócio no schema `auth` e `EmailMessage`/`ErrorLog` no schema `common` do banco `ouroboros_auth`. `AddCommon<AuthDbContext>` registra os serviços comuns; `AddEmailOutbox<AuthDbContext>` registra SMTP, dispatcher e background service dentro do processo de Auth.
...
Um host `Ouroboros.Services.Notifications.Api` comporta consumidor e dispatcher como serviços de fundo, além de saúde HTTP. Não há necessidade de separar API e worker em dois deploys agora. Quatro camadas, banco `ouroboros_notifications`, role `notifications_service`, schema `notifications`; `common.error_logs` permanece local também nesse banco.

BuildingBlocks fornece `Entity`, `AppDbContext`, erros, envelope/outbox e adapter de transporte reutilizável. Templates, destinatários, estados SMTP e tentativas pertencem a Notificações. Nenhum `ProjectReference` de Auth para camadas de Notifications ou vice-versa. Proposta: um pequeno projeto neutro `src/Contracts/Ouroboros.Contracts.Notifications`, contendo somente DTOs versionados, referenciado pelas Applications. A regra de dependências deve explicitar essa exceção para contratos de integração; não colocar entidades EF ou SDK do broker nesse projeto. Ele é código compartilhado, sem banco, e requer revisão como parte desta spec.
...
Manter uma instância PostgreSQL, com novo banco lógico. `NotificationsDbContext : AppDbContext`, `HasDefaultSchema("notifications")`, EF Core/Npgsql e snake_case; EF Design somente no projeto Api. `AddNotificationsModule` concentra persistência e casos de uso. Separar `AddCommon` de `AddTransactionalOutbox` e da entrega específica.

Criar script de provisionamento de banco/role em `docker/postgres/init/`, LF, e caminho administrativo repetível para instalações com volume existente: entrypoint de init não roda novamente só porque entrou um arquivo novo. Não exigir `down -v` nem recriação do banco. Aplicar migrations de Auth e Notifications separadamente pela ferramenta EF, mantendo a convenção atual de não migrar ao subir Compose.
...
Preservar migrations históricas: [InitialCreate](../src/Services/Auth/Ouroboros.Services.Auth.Infrastructure/Migrations/20260903001615_InitialCreate.cs), [navegações](../src/Services/Auth/Ouroboros.Services.Auth.Infrastructure/Migrations/20260903115155_AddTokenAndRefreshTokenNavigations.cs) e [tracking](../src/Services/Auth/Ouroboros.Services.Auth.Infrastructure/Migrations/20260903171415_AddEmailMessageDeliveryTracking.cs). Novas migrations fazem expansão e remoção em momentos diferentes; não reescrever passado.
...
| Alterar | [CommonModule.cs](../src/BuildingBlocks/Ouroboros.BuildingBlocks.Infrastructure/CommonModule.cs), [CommonEntityConfiguration.cs](../src/BuildingBlocks/Ouroboros.BuildingBlocks.Infrastructure/Persistence/CommonEntityConfiguration.cs), [csproj Infrastructure](../src/BuildingBlocks/Ouroboros.BuildingBlocks.Infrastructure/Ouroboros.BuildingBlocks.Infrastructure.csproj) | Separar registros/mapeamentos, adicionar cliente de broker escolhido e retirar MailKit no fim. `AppDbContext` permanece salvo necessidade comprovada do mapeamento. |
| Criar | `src/Services/Notifications/Ouroboros.Services.Notifications.Domain/`: csproj, `EmailDelivery.cs`, `EmailDeliveryAttempt.cs`, `EmailDeliveryStatus.cs` | Estados e regras de entrega. |
| Criar | `src/Services/Notifications/Ouroboros.Services.Notifications.Application/`: csproj, `Interfaces/IEmailDeliveryRepository.cs`, `IUnitOfWork.cs`, `IEmailSender.cs`, `IEmailTemplateRenderer.cs`, `Services/CreateEmailDeliveryService.cs`, `SendEmailDeliveryService.cs` | Casos de uso sem EF/AMQP/SMTP concreto. |
| Criar | `src/Services/Notifications/Ouroboros.Services.Notifications.Infrastructure/`: csproj, `NotificationsModule.cs`, `Persistence/NotificationsDbContext.cs`, `UnitOfWork.cs`, repositório/configurações, `Migrations/`, `Messaging/EmailNotificationConsumer.cs`, `Services/EmailDeliveryProcessor.cs`, `SmtpEmailSender.cs`, `EmailTemplateRenderer.cs`, `Options/`, `Templates/` | Consumer, envio, agendamento, banco e templates. MailKit fica aqui. |
| Criar | `src/Services/Notifications/Ouroboros.Services.Notifications.Api/`: csproj, `Program.cs`, `appsettings.json`, `Properties/launchSettings.json`, `Dockerfile` | Host, DI, saúde, telemetria e configuração sem segredos. |
| Alterar | [Token.cs](../src/Services/Auth/Ouroboros.Services.Auth.Domain/Token.cs), [UserRegistrationService.cs](../src/Services/Auth/Ouroboros.Services.Auth.Application/Services/UserRegistrationService.cs), [PasswordResetService.cs](../src/Services/Auth/Ouroboros.Services.Auth.Application/Services/PasswordResetService.cs), csproj Application Auth | UUID e contrato de solicitação; manter URLs/regras/atomicidade. |
| Alterar | [AuthDbContext.cs](../src/Services/Auth/Ouroboros.Services.Auth.Infrastructure/Persistence/AuthDbContext.cs), [AuthModule.cs](../src/Services/Auth/Ouroboros.Services.Auth.Infrastructure/AuthModule.cs), [snapshot](../src/Services/Auth/Ouroboros.Services.Auth.Infrastructure/Migrations/AuthDbContextModelSnapshot.cs), novas migrations de expansão/contração | Mapear outbox e UUID, manter legado até reconciliação. Revisar UnitOfWork para retry da transação não gerar novas solicitações involuntárias. |
| Alterar | [Program Auth](../src/Services/Auth/Ouroboros.Services.Auth.Api/Program.cs), [appsettings Auth](../src/Services/Auth/Ouroboros.Services.Auth.Api/appsettings.json), [csproj Infrastructure Auth](../src/Services/Auth/Ouroboros.Services.Auth.Infrastructure/Ouroboros.Services.Auth.Infrastructure.csproj), [Dockerfile Auth](../src/Services/Auth/Ouroboros.Services.Auth.Api/Dockerfile) | Trocar registros/configuração, retirar cópia de templates de e-mail, incluir novo projeto Contracts no restore Docker. |
...
Os testes existentes cobrem regras de `EmailMessage`, fila, dispatcher, cadastro, reset e tokens. [FakeUnitOfWork](../tests/Services/Auth/Ouroboros.Services.Auth.Application.Tests/Fakes/FakeUnitOfWork.cs) apenas conta chamadas e executa delegates; os testes EF usam InMemory. Eles não demonstram rollback real, constraints PostgreSQL, locking ou garantias do broker.
...
| BuildingBlocks.Infrastructure.Tests | Substituir [EmailQueueServiceTests](../tests/BuildingBlocks/Ouroboros.BuildingBlocks.Infrastructure.Tests/EmailQueueServiceTests.cs), [EmailOutboxDispatcherTests](../tests/BuildingBlocks/Ouroboros.BuildingBlocks.Infrastructure.Tests/EmailOutboxDispatcherTests.cs), ajustar `TestDbContext`; publisher com confirms/sem rota, lease, retomada e republicação do mesmo UUID. Mover fake SMTP para Notifications. |
| Auth.Domain.Tests | Ajustar [TokenTests](../tests/Services/Auth/Ouroboros.Services.Auth.Domain.Tests/TokenTests.cs) para UUID, mantendo uso/expiração/relações de negócio. |
...
Criar os quatro projetos de testes Notifications sob `tests/Services/Notifications/` e incluir na solution; contratos podem ser validados por fixtures JSON nos testes dos consumidores/produtores, sem depender da implementação do outro serviço. Integração com recursos reais deve ter execução documentada e determinística, separada dos unitários por categoria quando necessário.
...
`0001` (Git) e `0004` (EF/Dapper) não demandam alteração funcional identificada. Este documento separado é uma proposta de evolução para revisão; `0007` ainda descreve a implementação atual e não foi substituído por uma arquitetura ainda inexistente.
