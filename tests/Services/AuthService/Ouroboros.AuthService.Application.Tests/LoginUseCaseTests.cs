...
public class LoginUseCaseTests
{
	...
	public async Task LoginAsync_WithCorrectCredentials_ReturnsAccessTokenAndResetsAttempts()
	{
		...
		var loginUseCase = context.CreateLoginUseCase();

		var result = await loginUseCase.LoginAsync("jsilva", CorrectPassword, CancellationToken.None);
	...
	public async Task LoginAsync_WithUnknownLogin_ReturnsFailure()
	{
		...
		var result = await loginUseCase.LoginAsync("nao-existe", "any-password", CancellationToken.None);
	...
	public async Task LoginAsync_WithWrongPassword_ReturnsFailureAndRegistersAttempt()
	{
		...
		var result = await loginUseCase.LoginAsync("jsilva", "senha-errada", CancellationToken.None);
